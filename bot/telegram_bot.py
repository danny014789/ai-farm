...
schedules plant monitoring checks every 4 hours via APScheduler (integrated
...
async def scheduled_check(context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    Called every 4 hours by the JobQueue. Delegates to src.plant_agent.run_check()
...
def main() -> None:
    ...
    sets up the scheduled check (every 4h), and starts long-polling.
    """
    ...
    # --- Schedule monitoring check (every 4h) and heartbeat ----------------
    ...
    if job_queue is not None:
        ...
        logger.info("Scheduled monitoring check registered (every 14400s)")
